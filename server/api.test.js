...
function updateOrder(req,res){
    ...
    async.series([
        ...
        function(callback){
           ...
           models.instance.users.find({user_id:uuid},{select:['role']},function(err,user){
               listRole=(user && user[0].role && user[0].role.length >0 ) ? user[0].role : [];
               callback(null,null);
           })
        }, 
        function(callback){
            ...
            if(req.body.status){
                try{
                    if(listRole.indexOf('update_status') > -1 ){
                        ...
                        return res.send({status: 'invalid_rule_update_status'});
                    }
...
router.get("/",(req,res)=>{
    res.send({status: 'authentication fail'});
})
router.put('/order/update_order',updateOrder);
router.post('/order/list',getOrder);
router.post('/order/add',addOrder);
router.delete('/order/del_row',delOrder);
router.post('/user/check_account',checkAccount);
...
router.put('/user/add_rule',updateRule);
